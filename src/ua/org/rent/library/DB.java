...
public class DB extends SQLiteOpenHelper {
    ...
    private void createTables(SQLiteDatabase db) {
        final String[] items = mContext.getResources().getStringArray(R.array.DDL);
    ...
    private void deleteTables(SQLiteDatabase db) {
        Cursor cursor = db.query("sqlite_master", new String[]{"name"}, "type = ? AND NOT name = ? AND NOT name = ?", new String[]{"table","sqlite_sequence","android_metadata",}, null, null, null);
    ...
    public static void cleanAllEvents() {
        ...
        final Cursor c = db.rawQuery("SELECT name FROM sqlite_master WHERE type = 'table' " +
    ...
    public static void removeDeliveredEvents() {
        ...
        if (1==1)return;
       /*
        Cursor cursor = DB.getDb().query(Event.TABLE_NAME, new String[]{"max (timestamp)"}, "is_delivered = 1 AND type = ?", new String[]{String.valueOf(Const.BOX_BOLUS)}, null, null, null);
        Log.d("Cleaning delivered items","type "+Event.TABLE_NAME);
        if(cursor.moveToFirst()) {
            long maxtTimestamp = cursor.getLong(0);
            if(maxtTimestamp > 0) DB.getDb().delete("event", "is_delivered = 1 AND timestamp <= ? AND type <> ? ", new String[]{String.valueOf(maxtTimestamp),String.valueOf(Const.BOX_BOLUS)});
            long bolusMaxTmp = System.currentTimeMillis() - Format.hoursToMilliseconds(Settings.get().getSyncSettings().getUserProfile().getInt(UserValues.ai_time)) - Format.hoursToMilliseconds(3);
            DB.getDb().delete("event", "is_delivered = 1 AND type = ? AND timestamp < ?", new String[]{String.valueOf(Const.BOX_BOLUS), String.valueOf(bolusMaxTmp)});
        }
